...
// Browser auth client based on the uploaded client/auth.client.js + http.js.
(function () {
  'use strict';
  ...
  const isAuthenticatedPayload = (payload) => {
    ...
    if (payload.success === true && (payload.user || payload.data?.user)) return true;
    if (payload.data && payload.data.authenticated === true) return true;
    return false;
  };

  const getDisplayNameFromUser = (user) => {
    const safe = user && typeof user === 'object' ? user : getSavedUser();
    return (safe && (safe.username || safe.name || safe.email || safe.id || safe.userId || safe._id))
      ? String(safe.username || safe.name || safe.email || safe.id || safe.userId || safe._id)
      : '';
  };
  ...
  const request = async (path, options = {}) => {
    ...
    try {
      ...
      throw new Error('Could not connect to the No Read account system. Make sure the server and Cloudflare tunnel are online.');
    }
    ...
    if (!response.ok) {
      ...
      if (response.status === 401 || response.status === 403) {
        ...
        if (cleanPath.includes('/auth/login')) {
          message = 'Login was rejected by the account server. If the username/password are correct, your server is probably protecting /auth/login with auth middleware.';
        } else if (cleanPath.includes('/auth/register')) {
          message = 'Register was rejected by the account server. Check that /auth/register is public.';
        } else {
  ...
  const getSessionState = async ({ verify = true } = {}) => {
    ...
    if (!verify) {
      return {
        loggedIn: Boolean(savedUser || savedToken),
        user: savedUser,
        source: savedUser || savedToken ? 'local' : 'none'
      };
    }
  ...
  const api = {
    ...
    request,
    registerAccount: ({ username, password, email, dateOfBirth }) => request('/auth/register', {
      method: 'POST',
      body: { username, password, email, dateOfBirth }
    }),
    ...
    getSong: (id) => request(`/music/songs/${encodeURIComponent(id)}`),
    uploadSong: (payload) => request('/music/songs', { method: 'POST', body: payload }),
    startSongPlay: (id) => request(`/music/songs/${encodeURIComponent(id)}/play`, { method: 'POST' }),
    completeSongAd: (sessionId) => request(`/music/play-sessions/${encodeURIComponent(sessionId)}/complete-ad`, { method: 'POST' }),
    getArtist: (username) => request(`/music/artists/${encodeURIComponent(username)}`),
    adminListSongs: () => request('/music/admin/songs'),
    adminTakeDownSong: (id, reason) => request(`/music/admin/songs/${encodeURIComponent(id)}/take-down`, { method: 'POST', body: { reason } }),
    adminRestoreSong: (id) => request(`/music/admin/songs/${encodeURIComponent(id)}/restore`, { method: 'POST' }),
    adminBanUser: (id, reason) => request(`/admin/users/${encodeURIComponent(id)}/ban`, { method: 'POST', body: { reason } }),
    getMyUserProfile: () => request('/users/me'),
    updateMyUserProfile: (payload) => request('/users/me/profile', { method: 'PATCH', body: payload }),
